# RuntimeAuthorityCommand

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**agent_id** | **str** | Agent identifier for auditing | 
**json** | **bool** | Set output format to JSON | [optional] [default to False]
**name** | **str** | Secret name (dynamic secret or rotated secret) | 
**original_prompt** | **str** | Original natural-language prompt from the user (optional, for auditing) | [optional] 
**original_user** | **str** | Human end-user behind the agent (optional, for auditing) | [optional] 
**payload** | **str** | Query or action payload (SQL, natural language, or CLI-style) | 
**token** | **str** | Authentication token (see &#x60;/auth&#x60; and &#x60;/configure&#x60;) | [optional] 
**uid_token** | **str** | The universal identity token, Required only for universal_identity authentication | [optional] 

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


