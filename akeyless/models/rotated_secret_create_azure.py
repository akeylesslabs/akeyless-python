...
class RotatedSecretCreateAzure(BaseModel):
    """
    ...
    application_id: Optional[StrictStr] = Field(default=None, description="Id of the azure app that hold the serect to be rotated (relevant only for rotator-type=api-key & authentication-credentials=use-target-creds)", alias="application-id")
    ...
    rotator_type: StrictStr = Field(description="The rotator type. options: [target/password/api-key/azure-storage-account]", alias="rotator-type")
    ...
    username: Optional[StrictStr] = Field(default=None, description="The user principal name to rotate his password (relevant only for rotator-type=password)")
    __properties: ClassVar[List[str]] = ["api-id", "api-key", "application-id", "authentication-credentials", "auto-rotate", "delete_protection", "description", "json", "key", "max-versions", "name", "password-length", "rotate-after-disconnect", "rotation-event-in", "rotation-hour", "rotation-interval", "rotator-type", "secure-access-disable-concurrent-connections", "secure-access-enable", "secure-access-url", "secure-access-web", "secure-access-web-browsing", "secure-access-web-proxy", "storage-account-key-name", "tags", "target-name", "token", "uid-token", "username"]
