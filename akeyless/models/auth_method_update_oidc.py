...
class AuthMethodUpdateOIDC(BaseModel):
    """
    ...
    allowed_redirect_uri: Optional[List[StrictStr]] = Field(default=None, description="Allowed redirect URIs after the authentication", alias="allowed-redirect-uri")
    ...
    client_id: Optional[StrictStr] = Field(default=None, description="Client ID", alias="client-id")
    client_secret: Optional[StrictStr] = Field(default=None, description="Client Secret", alias="client-secret")
    ...
    required_scopes: Optional[List[StrictStr]] = Field(default=None, description="RequiredScopes is a list of required scopes that the oidc method will request from the oidc provider and the user must approve", alias="required-scopes")
    required_scopes_prefix: Optional[StrictStr] = Field(default=None, description="RequiredScopesPrefix is a a prefix to add to all required-scopes when requesting them from the oidc server (for example, azures' Application ID URI)", alias="required-scopes-prefix")
    ...
    unique_identifier: StrictStr = Field(description="A unique identifier (ID) value should be configured for OIDC, OAuth2, LDAP and SAML authentication method types and is usually a value such as the email, username, or upn for example. Whenever a user logs in with a token, these authentication types issue a \"sub claim\" that contains details uniquely identifying that user. This sub claim includes a key containing the ID value that you configured, and is used to distinguish between different users from within the same organization.", alias="unique-identifier")
    __properties: ClassVar[List[str]] = ["access-expires", "allowed-redirect-uri", "audience", "audit-logs-claims", "bound-ips", "client-id", "client-secret", "delete_protection", "description", "expiration-event-in", "force-sub-claims", "gw-bound-ips", "issuer", "json", "jwt-ttl", "name", "new-name", "product-type", "required-scopes", "required-scopes-prefix", "subclaims-delimiters", "token", "uid-token", "unique-identifier"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "allowed-redirect-uri": obj.get("allowed-redirect-uri"),
            ...
            "required-scopes": obj.get("required-scopes"),
            "required-scopes-prefix": obj.get("required-scopes-prefix"),
